...
class Env:
    ...
    def log(self, line):
        line = str(datetime.now()) + ": " + str(line)
    ...
    def compute_reward(self):
        ...
        # Reward uptill speed of 3, no reward after
        #if self.car_state.speed<3:
        #    reward+=self.car_state.speed/3

        top_speed = 1
        top_speed_reward = 10

        # Max reward at top_speed
        reward += (2*top_speed*self.car_state.speed - (self.car_state.speed**2))  / (top_speed**2) * top_speed_reward
        ...
        distance_pc = sorted(self.lidar_pc, key = lambda p: distance(p[0], p[1], 0,0))
        
        #print(distance(distance_pc[0][0], distance_pc[0][1], 0, 0), distance(distance_pc[-1][0], distance_pc[-1][1], 0, 0))
    ...
    def time_print(self):
        for fn in self.timers:
            self.log(fn, self.timers[fn]['finish'] - self.timers[fn]['start'])